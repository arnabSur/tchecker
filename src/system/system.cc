...
public:
  ...
  /*!
   \brief Add bounded parameter
   */
  virtual void visit(tchecker::parsing::param_declaration_t const & d)
  {
    if (std::numeric_limits<tchecker::param_id_t>::max() < d.size())
      throw std::out_of_range("parameter size cannot be represented");
    tchecker::param_id_t size = d.size();
    _system.add_param(d.name(), size, d.min(), d.max(), d.attributes());
  }
...
void system_t::add_param(std::string const & name, tchecker::param_id_t size, tchecker::integer_t min,
                          tchecker::integer_t max, tchecker::system::attributes_t const & attr)
{
  ...
  tchecker::system::params_t::add_param(name, size, min, max, attr);
}
